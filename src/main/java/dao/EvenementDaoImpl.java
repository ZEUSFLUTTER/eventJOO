...
public class EvenementDaoImpl implements EvenementDao {
    ...
    public List<Evenement> findAll() {
        return em.createQuery("SELECT e FROM Evenement e ORDER BY e.dateCreation DESC", Evenement.class)
                .getResultList();
    }
    ...
    public List<Evenement> findByOrganisateur(Long organisateurId) {
        TypedQuery<Evenement> query = em.createQuery(
                "SELECT e FROM Evenement e WHERE e.organisateur.id = :orgaId ORDER BY e.dateCreation DESC", 
                Evenement.class);
